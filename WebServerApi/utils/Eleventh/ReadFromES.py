...
class scan_data_fun(object):
    ...
    def get_battery_data(self, province, startTime, endTime, range):  # 获取电池数据
        ...
        try:
            if province == '全国':
                query_string = "A_source:vehicle_all_charge_table AND Time : [%s TO %s] AND charge_soc_l:[%s TO %s] | " \
                ...
                query_string = "A_source:vehicle_all_charge_table AND Time : [%s TO %s] AND T_vehicle_region:%s AND " \
                               "charge_soc_l:[%s TO %s] | report count(A_source) on charge_times_eachday" \
...
if __name__ == "__main__":
    conn = scan_data_fun()
    a, bo = conn.get_battery_data('鲁', '2017-01-01', '2017-01-31','00-99')
    for i in a:
