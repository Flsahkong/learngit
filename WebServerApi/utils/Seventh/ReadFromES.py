...
class scan_data_fun(object):
    ...
    def get_battery_data(self, province, startTime, endTime):  # 获取电池数据
        ...
        try:
            if province == '全国':
                query_string = "A_source:vehicle_all_drive_table AND Time : [%s TO %s] | report count(A_source) on " \
                               "mile_per_kWh,energy_all,VNO" % (startTime, endTime)
            else:
                query_string = 'A_source:vehicle_all_drive_table AND Time : [%s TO %s] AND T_vehicle_region:%s | ' \
                               'report count(A_source) on mile_per_kWh,energy_all,VNO' % (
...
if __name__ == "__main__":
    conn = scan_data_fun()
    a, bo = conn.get_battery_data('全国', '2017-01-01', '2018-01-31')
