...
class battery_original_data(object):
    def __init__(self, time="", B1="",B2=""):
        self.energy_all = time
        self.mile_per_kWh=B1
        self.VNO = B2
...
class scan_data_fun(object):
    ...
    def get_battery_data(self, province, startTime, endTime):  # 获取电池数据
        ...
        try:
            if province == '全国':
                query_string = "A_source:vehicle_all_drive_table AND Time : [%s TO %s]" % (startTime, endTime)
            else:
                query_string = "A_source:vehicle_all_drive_table AND Time : [%s TO %s] AND T_vehicle_region:%s" % (
            ...
            scan_fields = []
            scan_fields.append("energy_all")
            scan_fields.append("mile_per_kWh")
            ...
            while True:
                if result.lines:
                    for item_fields in result.lines:
                        batmodel = battery_original_data(item_fields['energy_all'],
                                                         item_fields['mile_per_kWh'],
                                                         item_fields['VNO']
                                                         )
...
if __name__ == "__main__":
    conn = scan_data_fun()
    a, bo = conn.get_battery_data('浙', '2017-01-01', '2017-01-31')

    for i in a:
        if i.mile_per_kWh!='-32000.0' and i.mile_per_kWh!='-32000':
            print(i.energy_all,i.mile_per_kWh, i.VNO)
